...
class Serializer(SerializerMixin, metaclass=abc.ABCMeta):
    ...
    def _build_seq_output(
        ...
        for idx in range(len(labels)):
            label = self.dataset.to_label(int(labels[idx]))
...
class GenRelationSerializer(Serializer):
    ...
    def _build_seq_input(self, data: ProcessedLayoutData) -> str:
        ...
        for idx in range(len(relations)):
            ...
            if label_i != 0:
                tokens.append(f"{self.dataset.to_label(int(label_i))} {index_i}")
            else:
            ...
            if label_j != 0:
                tokens.append(f"{self.dataset.to_label(int(label_j))} {index_j}")
            else:
                tokens.append("canvas")
